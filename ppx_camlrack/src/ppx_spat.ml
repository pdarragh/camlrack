...
let convert_case (c : case) : (case list) option =
  ...
  let convert_sexp_pattern ~loc (pat : sexp_pattern) : conditional_pattern list =
    ...
    and convert_subpats (pats : sexp_pattern list) (add_var_cpat : bool) : conditional_pattern list =
      let build_replicated_patterns
          ...
          (pat : pattern)
          (conds : expression list) : (pattern list * expression list) list =
        (* Produces a list of patterns corresponding to 0-`len` repetitions of the
           given pattern. *)
        let rec replicate_pattern
          ...
          then
            ...
            replicate_pattern pat (sexp_pat :: pats) (len - 1) in
      ...
      let rec convert_subpats'
        ...
        | sexp_pat :: PSymbol "..." :: sexp_pats' ->
          ...
          let replicated_expansions : (pattern list * expression list) list =
            List.flatten
              (List.map
                 (fun (p, conds) -> build_replicated_patterns sexp_pat p conds)
                 pat_expansions) in
  ...
  let build_guard ~loc (conds : expression list) : expression option =
    ...
    | _ ->
      ...
      (match c.pc_guard with
       ...
       | None -> Some anded_conds) in
  let convert_cond_pat_to_cases ~loc ((pat, conds) : conditional_pattern) : case =
    case ~lhs:pat ~guard:(build_guard ~loc conds) ~rhs:c.pc_rhs
  in
